...
SYSVER=$($SYSCOINCLI --version | head -n1 | awk -F'[ -]' '{ print $6 }')
SYSCOMID=$($SYSCOINCLI --version | head -n1 | awk -F'[ -]' '{ print $7 }')
...
for cmd in $SYSCOIND $SYSCOINCLI $SYSCOINTX $SYSCOINQT; do
  cmdname="${cmd##*/}"
  $(help2man -N --version-string=$SYSVER --include=footer.h2m -o ${MANDIR}/${cmdname}.1 ${cmd})
  sed -i "s/\\\-${SYSCOMID}//g" ${MANDIR}/${cmdname}.1
done
