...
UniValue escrowrelease(const UniValue& params, bool fHelp) {
	...
	if(role == "arbiter")
	{
		...
		if(arbiterAliasLatest.multiSigInfo.vchAliases.size() > 0)
			scriptPubKeyOrig = CScript(arbiterAliasLatest.multiSigInfo.vchRedeemScript.begin(), arbiterAliasLatest.multiSigInfo.vchRedeemScript.end());
...
UniValue escrowclaimrefund(const UniValue& params, bool fHelp) {
	...
	CAliasIndex sellerAlias, sellerAliasLates, buyerAlias, buyerAliasLatestt;
	vector<CAliasIndex> aliasVtxPos;
