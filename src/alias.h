...
public:
	...
	bool WriteAlias(const std::vector<unsigned char>& name, const CAliasUnprunable &aliasUnprunable, const std::vector<unsigned char>& address, const std::vector<unsigned char>& address1,const std::vector<CAliasIndex>& vtxPos) {
		if(address.empty())
			return false;	
		if(!address1.empty())
			 Write(make_pair(std::string("namea"), address1), name);
...
void CreateFeeRecipient(const CScript& scriptPubKey, const std::vector<unsigned char>& vchAliasPeg, const uint64_t& nHeight, const std::vector<unsigned char>& data, CRecipient& recipient);
...
unsigned int aliasselectpaymentcoins(const std::vector<unsigned char> &vchAlias, const CAmount &nAmount, std::vector<COutPoint>& outPoints, bool& bIsFunded, CAmount &nRequiredAmount, bool bSelectFeePlacementOnly);
