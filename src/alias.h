...
unsigned int aliasselectpaymentcoins(const std::vector<unsigned char> &vchAlias, const CAmount &nAmount, unsigned int &numCoinsLeft, std::vector<COutPoint>& outPoints, bool& bIsFunded, CAmount &nRequiredAmount, bool bSelectFeePlacementOnly, bool bSelectAll=false);
