...
UniValue offerlink(const UniValue& params, bool fHelp) {
	if (fHelp || params.size() < 3 || params.size() > 4)
		throw runtime_error(
		"offerlink <alias> <guid> <commission> [details]\n"
						"<alias> An alias you own.\n"
...
UniValue offerupdate(const UniValue& params, bool fHelp) {
	...
	if(offerCopy.vchLinkOffer.empty())
	{
		...
		int precision = 2;
		nPricePerUnit = convertCurrencyCodeToSyscoin(alias.vchAliasPeg, sCurrencyCode.empty()? offerCopy.sCurrencyCode: sCurrencyCode, fPrice, chainActive.Tip()->nHeight, precision);
		if(nPricePerUnit == 0)
		{
...
void OfferTxToJSON(const int op, const std::vector<unsigned char> &vchData, const std::vector<unsigned char> &vchHash, UniValue &entry)
{
	...
	string detailsValue = noDifferentStr;
	if(!offer.sDetails.empty() && offer.sDetails != dbOffer.sDetails)
		detailsValue = stringFromVch(offer.sDetails);
	entry.push_back(Pair("details", detailsValue));
